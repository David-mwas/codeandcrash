...
export class GameManager {
  ...
  constructor(canvas: HTMLCanvasElement, saveData: SaveData, options: GameManagerOptions) {
    this.canvas = canvas
    ...
    const upgrades = this.saveData?.permanentUpgrades || getDefaultSaveData().permanentUpgrades
  ...
  enemyKilled(enemy: Enemy) {
    ...
    const xpGain = Math.floor(enemy.xpValue * comboMultiplier * this.upgrades.xpBonus)
    ...
    const dropChance = 0.3 + (this.saveData?.permanentUpgrades?.luckyDrops || 0) * 0.05
    if (Math.random() < dropChance) {
      this.powerUps.push(new PowerUp(enemy.x, enemy.y))
    }

    // Check wave complete
    if (this.enemiesKilledThisWave >= this.totalEnemiesThisWave && this.enemiesSpawned >= this.totalEnemiesThisWave) {
  ...
  collectPowerUp(powerUp: PowerUp) {
    ...
    const allUpgrades = [
      ...
      { id: "maxHealth", name: "+20 Max Health", description: "More survivability" },
      { id: "bulletSpeed", name: "+15% Bullet Speed", description: "Faster projectiles" },
      { id: "reloadSpeed", name: "+20% Reload Speed", description: "Reload quicker" },
      { id: "pierce", name: "+1 Pierce", description: "Bullets go through enemies" },
      { id: "dashCooldown", name: "-15% Dash Cooldown", description: "Dash more often" },
    ]
  ...
  update(deltaTime: number) {
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (bullet.x < -50 || bullet.x > this.canvas.width + 50 || bullet.y < -50 || bullet.y > this.canvas.height + 50) {
      ...
      for (let j = this.enemies.length - 1; j >= 0; j--) {
        ...
        if (dist < bullet.radius + enemy.radius) {
          ...
          const lifestealAmount = actualDamage * ((this.saveData?.permanentUpgrades?.lifesteal || 0) * 0.02)
          if (lifestealAmount > 0) {
            this.player.health = Math.round(Math.min(this.player.health + lifestealAmount, this.player.maxHealth))
          }
          ...
          if (bullet.explosive && bullet.explosionRadius) {
            ...
            const radius =
              bullet.explosionRadius * (1 + (this.saveData?.permanentUpgrades?.explosionRadius || 0) * 0.15)
    ...
    for (const grenade of this.grenades) {
      ...
      if (grenade.exploded) {
        ...
        const explosionRadius = 100 + (this.saveData?.permanentUpgrades?.explosionRadius || 0) * 20
        for (const enemy of this.enemies) {
          const dx = enemy.x - grenade.x
          const dy = enemy.y - grenade.y
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist < explosionRadius) {
            const damage = 50 * (1 - dist / explosionRadius) * this.upgrades.damage
    ...
    for (const enemy of this.enemies) {
      ...
      if (dist < this.player.radius + enemy.radius - 2) {
        ...
        const armorReduction = 1 - (this.saveData?.permanentUpgrades?.armor || 0) * 0.05
    ...
    this.bullets = this.bullets.filter(
      (b) => !b.dead && b.x > -50 && b.x < this.canvas.width + 50 && b.y > -50 && b.y < this.canvas.height + 50,
    )
    ...
    if (this.isTutorial && this.onTutorialStep) {
      if (this.tutorialStep === 0 && (this.keys["w"] || this.keys["a"] || this.keys["s"] || this.keys["d"])) {
        this.tutorialStep = 1
        ...
        this.tutorialStep = 4
        this.onTutorialStep(4, "Excellent! Kill the remaining enemies to complete the tutorial")
      } else if (this.tutorialStep === 4 && this.enemies.length === 0 && this.enemiesKilledThisWave >= 2) {
        this.tutorialStep = 5
  ...
  // Methods for external control (mobile touch)
  setMobileMovement(dx: number, dy: number) {
    this.keys["w"] = dy < -0.3
    this.keys["s"] = dy > 0.3
    this.keys["a"] = dx < -0.3
    this.keys["d"] = dx > 0.3
  ...
  setMobileAim(x: number, y: number) {
    this.mouse.x = x
    this.mouse.y = y
  }
  ...
  onLevelUp() {
    ...
    if (this.showUpgradeMenu) {
      ...
      const upgradePool = [
        ...
        { id: "damage", name: "POWER BOOST", description: "+15% Damage" },
        { id: "moveSpeed", name: "SWIFT STEP", description: "+10% Move Speed" },
        { id: "bulletSpeed", name: "VELOCITY", description: "+15% Bullet Speed" },
        { id: "reloadSpeed", name: "QUICK RELOAD", description: "+20% Reload Speed" },
        { id: "pierce", name: "PIERCING", description: "Bullets Pierce +1 Enemy" },
        { id: "dashCooldown", name: "AGILITY", description: "-15% Dash Cooldown" },
      ]
